% Use this program to reproduce Fig. 13.18 and Fig. 13.20 of text.
close all
clear all
...
snrCI = snr_ref + 40*log10(R_ref_ci ./ range); %
% plot SNR versus range
figure(1)
rangekm  = range ./ 1000;
plot(rangekm,snr1,'k-.',rangekm,snrCI,'k','linewidth',1.5)
axis tight
grid
legend('\bfsingle pulse','94 pulse CI')
xlabel ('\bfDetection range - km');
ylabel ('\bfSNR - dB');
% Generate Figure 1.20
% first find the new reference range
snr_b10 = 10.^(10/10);
SNR_1 = 10/(2*94) + sqrt(((10^2) / (4*94*94)) + (10 / 94)); % 
LNCI = (1+SNR_1) / SNR_1; % 
%snrnci_ref = snrCI -10*log10(LNCI);
NCIgain = 10*log10(94) - 10*log10(LNCI);
R_ref_nci = ((10^(0.1*NCIgain))^0.25) * 2.245e3;
snrNCI = snr_ref + 40*log10(R_ref_nci ./ range);
figure (2)
plot(rangekm,snr1,'k',rangekm,snrNCI,'k -.', rangekm,snrCI,'k:','linewidth',1.5)
axis tight
grid
legend('\bfsingle pulse','94 pulse NCI','94 pulse CI')
xlabel ('\bfDetection range - km');
ylabel ('\bfSNR - dB');
