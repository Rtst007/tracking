...
figure('units', 'normalized', 'position', [0.1, 0.10, 0.25, 0.5]);
subplot(2,1,1)
% H(f) Average
ave2 = abs((X1 + X2 + X3 + X4)./4);
Have2 = 10*log10(abs((X1 + X2 + X3 + X4)./4));
plot(v2,Have2,'linewidth',1);
axis([0 1345 -25 5]);
title('\bf Two pulse MTI stagger ratio 25:30:27:31');
xlabel('\bf Radial Velocity (m/s)');
ylabel('\bfMTI Gain (dB)');
grid on
% %Mean value of H(f)
v4 = v2; 
f4 = (2.*v4)/.0375;
...
% Plot of H(f) Average
subplot(2,1,2)
plot(v4,Have4,'linewidth',1);
axis([0 1345 -25 5]);
title('\bf Four pulse MTI stagger ratio 25:30:27:31');
xlabel('\bfRadial Velocity (m/s)');
ylabel('\bf MTI Gain (dB)');
grid on
